...
class PGN(nn.Module):
    ...
    def load_model(self):
        ...
        if (os.path.exists(config.encoder_save_name)):
            ...
            self.encoder = torch.load('../saved_model/pgn/encoder.pt', map_location=torch.device('cpu'))
            self.decoder = torch.load('../saved_model/pgn/decoder.pt', map_location=torch.device('cpu'))
            self.attention = torch.load('../saved_model/pgn/attention.pt', map_location=torch.device('cpu'))
            self.reduce_state = torch.load('../saved_model/pgn/reduce_state.pt', map_location=torch.device('cpu'))
